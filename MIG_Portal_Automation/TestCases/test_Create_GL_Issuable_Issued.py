...
from TestElements import Common_HomeScreen, Common_SearchScreen, Common_NewAccountDetailsScreen, Common_AccountSummaryScreen, \
    Common_StartAQuoteScreen, GL_CoveragePartsScreen, Common_BasicPolicyInfoScreen, \
    Common_LocationsScreen, Common_LossHistoryScreen, Common_PolicyReviewScreen, Common_QuoteSummaryScreen, \
    Common_ReferToUnderwriterScreen, Common_PCScreen, GL_CGLScreen, Common_PayAndIssueScreen

class CreateGLIssuableIssued(MyUnitTest.MyTest):

    def test_new_gl_Issuable_issued(self):

        # Create screenshot excel
        book = xlsxwriter.Workbook('..\MIG_Portal_Automation\TestReports\Create_GL_Issuable_Issued.xlsx')

        # Load test data
        wb = load_workbook('..\MIG_Portal_Automation\TestData/test_Create_GL_Issuable_Issued.xlsx')
        ws = wb['GLIssued']
        ...
        cgl.click_supplementalinfo_tab()
        ...
        Functions.get_screenshot(self.driver, book, "LossHistory")
